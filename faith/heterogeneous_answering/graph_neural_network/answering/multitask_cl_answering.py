...
from faith.heterogeneous_answering.graph_neural_network.answering.answering import Answering
torch.autograd.set_detect_anomaly(True)


class MultitaskCLAnswering(Answering):
    def __init__(self, config):
        super(MultitaskCLAnswering, self).__init__(config)
    ...
    def forward(self, batch, train, entity_mat, sr_vec, ev_mat, iteration=None, *args):
        ...
        answer_logits = answer_logits.clamp(min=1e-30)
        ...
        ev_logits = ev_logits.clamp(min=1e-30)
