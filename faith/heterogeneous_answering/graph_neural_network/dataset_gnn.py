...
def tsf_dic_to_string(tsf, delimiter):
    if isinstance(tsf, dict):
        if "entity" in tsf and "relation" in tsf:
            ...
            query = f"{entity} {delimiter} {relation} {delimiter} {answer_type}"
    else:
        query = tsf
...
class DatasetGNN(Dataset):
    ...
    def prepare_turn(config, turn, train=False):
        # load data
        tsf = tsf_dic_to_string(turn["structured_temporal_form"], config["tsf_delimiter"])
        question = turn["question"]
        evidences = turn["candidate_evidences"]
        ...
        if train and "Temporal question type" in turn and config.get("gnn_drop_implicit", False) and "Implicit" in turn["Temporal question type"]:
            raise ContinueWithNext("Skipping implicit questions during training.")
