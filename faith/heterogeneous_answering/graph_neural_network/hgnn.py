...
class HeterogeneousGNN(torch.nn.Module):
    """
    Heterogeneous graph neural network, as used in faith (SIGIR 2023 full paper).
    - Node-type specific projections
    - Attention on SR (in faith, SR is TSF)
    """
    ...
    def forward(self, batch, train=False):
        ...
        entities_mat = self.encoder.encode_entities_batch(
            entities, tsfs, evidences_mat, ev_to_ent, tsf_vec
        ...
        for i in range(self.num_layers):
            ...
            ent_att_scores = torch.bmm(projected_ents, tsf_vec.unsqueeze(dim=2)) # size: batch_size x num_ent x 1
