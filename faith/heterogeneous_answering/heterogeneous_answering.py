...
class HeterogeneousAnswering:
    ...
    def _calculate_statistics(self, data):
        ...
        for instance in data:
            # 检查RAG答案
            if "rag_answer" in instance:
                if instance["rag_answer"] == "证据不足":
                    stats["no_evidence_instances"] += 1
                elif instance["rag_answer"] == "生成答案时出错":
                    stats["error_instances"] += 1
                else:
                    stats["answered_instances"] += 1
    ...
    def get_module_class(module_type):
        """根据模块类型返回相应的类"""
        if module_type == "faithful":
            from faith.heterogeneous_answering.faithful_answering.faithful_answering_module import FaithfulAnsweringModule
            return FaithfulAnsweringModule
        elif module_type == "rag":
            from faith.heterogeneous_answering.rag_answering.rag_answering_module import RAGAnsweringModule
            return RAGAnsweringModule
        else:
            raise ValueError(f"不支持的异构答案生成模块类型: {module_type}")
            
    @staticmethod
    def create(config):
        """创建异构答案生成模块实例"""
        ha_type = config.get("ha_type", "faithful")
        logger = get_logger("HeterogeneousAnswering", config)
        logger.info(f"创建异构答案生成模块: {ha_type}")
        
        try:
            module_class = HeterogeneousAnswering.get_module_class(ha_type)
            return module_class(config)
        except Exception as e:
            logger.error(f"创建异构答案生成模块失败: {str(e)}")
            raise
