...
class TemporalQuestionUnderstanding:
    ...
    def inference(self):
        ...
        self.inference_on_data_split("dev")
        self.inference_on_data_split("train")
        self.inference_on_data_split("test")
    ...
    def inference_on_data_split(self, split):
        tqu = self.config["tqu"]
        ...
        self.inference_on_data(dataset)
    ...
    def inference_on_data(self, input_data, es_method, ha_method, sources=["kb", "text", "table", "info"]):
        """Run model on data and add predictions."""
        # model inference on given data
        for instance in tqdm(input_data):
            self.inference_on_instance(instance, es_method, ha_method, sources)
        return input_data

    def inference_on_instance(self, instance, es_method, ha_method, sources=["kb", "text", "table", "info"]):
        raise Exception(
