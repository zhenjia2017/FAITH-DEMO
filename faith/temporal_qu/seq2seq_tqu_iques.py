...
class Seq2SeqIQUESTQU(TemporalQuestionUnderstanding):
    ...
    def __init__(self, config, pipeline, benchmark):
        self.config = config
        ...
        self.seq2seq_tsf = Seq2SeqTSFModule(config, benchmark)
        # initiate seq2seq model class for generating intermedaite question in implicit question resolver
        self.seq2seq_iques = Seq2SeqIQUESModule(config, benchmark)
        ...
        if self.run_tvr:
            ...
            self.tvr = TemporalValueResolver(config, pipeline, self.seq2seq_iques)
    ...
    def inference_on_instance(self, instance, es_method, ha_method, sources):
        ...
        if qtemporal_type == "implicit" and self.run_tvr:
            # implicit resolver
            temporal_value, iques_instance = self.tvr.resolve_implicit_temporal_value(instance, es_method, ha_method, sources)
            if temporal_value:
...
if __name__ == "__main__":
    ...
    # inference: add predictions to data
        ...
        instance = ''
        tqu = TemporalQuestionUnderstanding(config)
        res = tqu.inference_on_instance(instance)
        print(res)
