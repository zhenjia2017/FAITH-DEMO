...
class TemporalValueResolver:
    def __init__(self, config, pipeline, iques_generation):
        self.config = config
        self.logger = get_logger(__name__, config)
        # instantiate fine-tuned BART (Seq2Seq) model to generate intermediate question
        self.iques_generation = iques_generation
        ...
        self.tvr_topk_answer = config["tvr_topk_answer"]

    def resolve_implicit_temporal_value(self, instance, fer_scoring_method, ha_method, sources):
        iques_answers = []
        temporal_values = []
            ...
            if generated_q.lower() != instance["Question"].lower():
                ...
                if answer_type == "time interval":
                    ...
                    for item in suffix:
                        ...
                        iques_instance.update({"faith_or_unfaith": instance["faith_or_unfaith"]})
                        iques_instance.update({"iteration": instance["iteration"]})
                        iques_instance.update({"gnn_max_output_evidences": instance["gnn_max_output_evidences"]})
                        ...
                        iques_instance = self.pipeline.inference_on_instance(iques_instance, fer_scoring_method, ha_method, sources)
                        ranked_answers = iques_instance["ranked_answers"]
                        # obtain the top-k answer of temporal value
                        temporal_value = self.extract_temporal_value(ranked_answers)  # extract temporal value from answer
                        # remember results
    ...
    def extract_temporal_value(self, ranked_answers):
        ...
        if pred_answers:
            for answer in pred_answers:
                if answer["rank"] > self.tvr_topk_answer:
                    self.logger.info(
