...
class ClocqRetriever:
    def __init__(self, config, temporal_value_annotator):
        ...
        if self.use_cache:
            ...
            self.cache_path = os.path.join(config["path_to_data"], config["er_cache_path"])
            self._init_cache()
    ...
    def _kb_fact_to_evidence(self, kb_fact, question_items_set):
        ...
        def _format_fact(kb_fact):
            ...
            for item in kb_fact:
                ...
                if self.library.is_timestamp(item["id"]):
                    ...
                    if (item["label"], item["id"]) not in disambiguation:
                        ...
                        timeexpressions.append(item["label"])
                    if item_pre["id"] == 'P580':
                        # start time
            ...
            return kb_fact, timespan, disambiguation, timeexpressions, retrieved_for
        ...
        # evidence text
        kb_fact, timespan, disambiguation, timeexpressions, retrieved_for = _format_fact(kb_fact)
        ...
        evidence = {
            ...
            "retrieved_for_entity": retrieved_for,
            "tempinfo": [timespan, disambiguation, timeexpressions] if timespan and disambiguation else None,
            "source": "kb",
        }
        return evidence
