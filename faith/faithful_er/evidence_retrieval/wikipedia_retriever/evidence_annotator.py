...
class EvidenceAnnotator:
    ...
    def annotate_wikidata_entities(self, wiki_path, evidences, doc_anchor_dict):
        ...
        for evidence in new_evidences:
            ...
            timeexpressions = []
            if timespans and disambiguations:
                for span in evidence_texts_dates[new_evidences.index(evidence)][3]: 
                    timeexpression_start = span[0]
                    timeexpression_end = span[1]
                    # print (type(timeexpression_start))
                    # print (timeexpression_start)
                    # print (timeexpression_end)
                    # print (evidence["evidence_text"])
                    timeexpressions.append(evidence["evidence_text"][timeexpression_start:timeexpression_end])
            ...
            evidence["tempinfo"] = (
                [timespans, disambiguations, timeexpressions] if timespans and disambiguations else None
            )
    ...
    def _extract_dates_multithread(self, evidence_texts):
        ...
        for evidence_result in date_annotation_results:
            ...
            evidence_texts_dates.append([dates, timespan, disambiguations, span])
